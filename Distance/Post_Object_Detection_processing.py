...
clone = cropped_img.copy()

conts, hi_arc= cv2.findContours(cropped_img,cv2.RETR_TREE,cv2.CHAIN_APPROX_SIMPLE)
segmented = max(conts, key=cv2.contourArea)

cv2.drawContours(clone, [segmented + (y1, x2)], -1, (255, 0, 255))

cv2.namedWindow('contour image', cv2.WINDOW_NORMAL)
cv2.resizeWindow('contour image', window_width, window_height)
plt.imshow(clone)
#cv2.imshow('contour image',clone)

#cv2.waitKey(0)           
#if key == (27, ord("q")):
#    cv2.destroyAllWindows('contour image')
...
print("X2 is  ",x2) #displacement along x axis in meters
